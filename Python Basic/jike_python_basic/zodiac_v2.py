...
(input_month, input_day) = (int(input('birth month')), int(input('birth day')))

zodiac_index = 0
for month, day in zodiac_days:
    if input_month == month:
        if input_day >= day:
            zodiac_index = month
            if zodiac_index >= len(zodiac_days):
                zodiac_index = zodiac_index - len(zodiac_days)
        else:
            zodiac_index = month - 1
        break;
